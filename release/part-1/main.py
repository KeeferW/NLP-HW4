...
def create_augmented_dataloader(args, dataset):
    ...
    transformed_train = small_train.map(custom_transform, load_from_cache_file=False)

    combined = concatenate_datasets([dataset["train"], transformed_train])
    combined_tokenized = combined.map(tokenize_function, batched=True, load_from_cache_file=False)
